...
public class NewsRepository : INewsRepository
{
    ...
    public NewsRepository(LibraryManagementDbContext context, ILogger<INewsRepository> logger)
    {
        _context = context;
        _logger = logger;
    }
    ...
    public async Task<IEnumerable<NewsViewModel>> GetAll()
    {
        try
        {
            IEnumerable<NewsViewModel> item = await _context.news.AsNoTracking().Select(p=> new NewsViewModel(p)).ToListAsync();
            if (item != null)
            {
                return item;
            }
    ...
    public async Task<NewsViewModel> FindById(string id)
    {
        try
        {
            NewsViewModel? item = await _context.news.AsNoTracking().Where(p=> p.Id == id).Select(p=> new NewsViewModel(p)).FirstOrDefaultAsync();
            if (item != null)
            {
                return item;
            }
    ...
    public bool Update(string Id, NewsCreateViewModel viewModel)
    {
        try
        {
            ...
            if (item != null)
            {
                ...
                item.isPublish = viewModel.isPublish;
    ...
    public async Task<IEnumerable<NewsViewModel>> getLatestNews(PaginationRequest request)
    {
        try
        {
            ...
            IEnumerable<NewsViewModel> item = await _context.news.AsNoTracking().Where(p=> p.isPublish == true).OrderByDescending(p=> p.CreatedAt).Skip(skip).Take(request.PageSize).Select(p=> new NewsViewModel(p)).ToListAsync();
            if (item != null)
            {
                return item;
            }
